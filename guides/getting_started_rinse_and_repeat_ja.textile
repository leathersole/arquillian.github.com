...
このガイドでは、とても基本的なショッピング・アプリケーションのバックエンドを作成します。訪問者が選んだ品物を保存するためにbasketコンポーネントを使い、確定した注文を保存し読み出すためにorder repositoryコンポーネントを使います。ここでは、それぞれをCDIとEJBを使って実装します。すでにJava EE 6 APIがクラスパス上にあるので、すぐにこれら二つのプログラミングモデルを利用して始めることができます（このAPIをプロジェクトに導入する手順は、 "Getting Started":/guides/getting_started ガイドを参照してください）。

オーダーを永続的なストレージとやりとりするコンポーネントから始めましょう。 Following good software design, and to simplify testing, we'll begin by defining an interface that defines the contract. In your IDE, create a new local EJB interface named @OrderRepository@ and populate it with the following contents:
